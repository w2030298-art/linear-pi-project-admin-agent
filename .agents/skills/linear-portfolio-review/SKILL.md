...
5. If writing is needed, call `linear_build_write_plan` once for this Project only. It builds the write plan, runs final validation, asks for one `plan_confirmation`, applies immediately only after approval, then reads back and audits. Use `linear_validate_and_apply_write_plan` only when starting from an existing write plan file.
