...
export function createNonInteractivePlanConfirmationResult(inputs: PlanConfirmationInputs = { writePlanPath: "", idempotencyKey: "" }) {
  return {
    ...
    openQuestions: [
      "Real Linear write is blocked until the same write gate can show plan_confirmation for the exact write plan."
    ]
  };
}
...
export async function runPlanConfirmationFlow(ctx: RepoMapAskContext, inputs: PlanConfirmationInputs) {
  ...
  if (choice === "No" || !choice) {
    return {
      ...
      openQuestions: ["Generate a new write plan or revise the operation input, then rerun the same write gate before real apply."]
    };
  }
  ...
  if (choice === "调整意见") {
    ...
    return {
      ...
      nextActions: ["Rewrite the operation input or write plan from the adjustment feedback, then rerun the same write gate."]
    };
  }
...
export default function (pi: ExtensionAPI) {
  pi.registerTool({
    ...
    promptGuidelines: [
      ...
      "For normal writes, do not call plan_confirmation directly; call linear_build_write_plan for structured operations or linear_validate_and_apply_write_plan for an existing write plan file. Those tools invoke this flow internally after final validation passes.",
      "If plan_confirmation returns revision_requested, rewrite the operation input or write plan from feedback, then rerun the same write gate.",
      "If plan_confirmation returns cancelled or interactive_confirmation_unavailable, do not apply real Linear mutations.",
      "When plan_confirmation returns approved inside the write gate, that same tool immediately applies the write plan. Do not show a second confirmation UI.",
