...
When a single-Project task has no explicit target, the first clarification step is `pi_ask_user(flow=project_select)`. Candidate options come from the local three-source repo-map only; Linear project context is read after the user selects a project ID or enters a custom value.
