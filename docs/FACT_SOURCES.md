...
- Use `linear_build_write_plan` for the normal structured write flow; use `linear_validate_and_apply_write_plan` only when starting from an existing write plan file. Pass `--workspace-manifest <path>` to `scripts/plan-reviewer.mjs` only for standalone local reviewer diagnostics.
