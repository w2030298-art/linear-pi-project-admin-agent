...
assert.equal(fs.existsSync(readmePath), true, `${readmePath} should exist`);
...
for (const text of [guide, localLaunch, report]) {
  ...
  assert.match(text, /runtime clone|runtime root/i);
...
for (const text of [guide, localLaunch, readme]) {
  assert.match(text, /managed runtime clone|runtime clone|managed runtime copy/i);
  assert.doesNotMatch(text, /runtime checkout|development checkout|checked-out source repo/i);
}
...
assert.match(installer, /'switch'/i);
assert.doesNotMatch(installer, /Invoke-CheckedCommand 'git' @\([^)]*'checkout'/is);
